...
fun PluviaMain(
                ...
                is MainViewModel.MainUiEvent.OnLogonEnded -> {
                    when (event.result) {
                        LoginResult.Success -> {
                            ...
                            // Instead, ask the user to contribute their issues.
                            if (!state.annoyingDialogShown && state.hasCrashedLastStart) {
                                ...
                                msgDialogState = MessageDialogState(
                                    ...
                                    message = "Thank you for using Pluvia, " +
                                        "please consider supporting us by tipping whatever amount is comfortable to you",
                                    confirmBtnText = "Tip",
    ...
    PluviaTheme {
        ...
        NavHost(
            ...
            /** Full Screen Chat **/
            // TODO: Oxters requested that due to adaptive compose being split-pane when in landscape mode
            //  that there should be a button (or maybe preference) to enter into a dedicated chat screen.
            ...
            composable(route = PluviaScreen.XServer.name) {
                XServerScreen(
                    ...
                    onExit = { viewModel.exitSteamApp(context, state.launchedAppId) },
                )
            }
...
fun preLaunchApp(
    ...
    CoroutineScope(Dispatchers.IO).launch {
        ...
        val imageFsInstallSuccess = ImageFsInstaller.installIfNeededFuture(context, context.assets) { progress ->
